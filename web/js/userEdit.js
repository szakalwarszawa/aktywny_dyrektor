$(document).ready(function() {
    ...
    var sekcja = document.getElementById(
        'parp_mainbundle_edycjauzytkownika_info',
    );
    ...
    function constrainVisibleOptGroups() {
        ...
        for (var i = 1; i < optgroups.length; i++) {
            ...
            for (var j = 0; j < exceptions.length; j++) {
                // pętla po tablicy wyjątków
                if (selectedDepartament.text !== exceptions[j]) {
                    if (
                        optgroups[i].label.indexOf(selectedDepartament.text) ===
                        -1
                    ) {
                        if (
                            optgroups[i].classList.contains('hidden') === false
                        ) {
